...
#if defined (CONFIG_MACH_MSM8960_SIRIUSLTE) //subamp gain control
...
{
	long ret = 0;
#if defined(CONFIG_PANTECH_SND) && defined(CONFIG_MACH_MSM8960_VEGAPVW)
	int data = 0;
...
#endif
#if defined (CONFIG_MACH_MSM8960_SIRIUSLTE) //Sub Amp Gain Control
	switch (cmd) {
...
subsys_initcall(pantech_audio_init); // subamp gain control
#else
